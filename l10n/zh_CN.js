OC.L10N.register(
    ...
    "Let user can register directly without email verification" : "让用户可以直接注册而无需电子邮件验证",
    "Allowed email domains" : "允许电子邮件域名",
    "Enter a semicolon-separated list of allowed email domains, * for wildcard. Example: %s" : "输入允许的电子邮件域名的分号分隔列表，* 表示通配符。 例如：%s",
    ...
    "If configured, usernames will be validated through the regular expression. If the validation fails the user is prompted with a generic error. Make sure your regex is working correctly." : "如果进行了配置，用户名将通过正则表达式进行验证。如果验证失败，则提示用户一个通用错误。请确保你的正则表达式工作正常。",
    ...
    "Add additional user instructions (e.g. for choosing their usernames). If configured the text is embedded in the the verification-Email." : "添加额外的用户指令(例如选择用户名)。如果配置好，该文本会嵌入在验证电子邮件中。",
    "Admin approval" : "管理员批准",
    "Require admin approval" : "需要管理员批准",
    "Enabling \"admin approval\" will prevent registrations from mobile and desktop clients to complete as the credentials can not be verified by the client until the user was enabled." : "启用 \"管理员批准” 将阻止从移动客户端和桌面客户端完成注册，因为在启用用户之前，客户端无法验证凭据。",
    ...
    "Enter a semicolon-separated list of allowed domains. Example: owncloud.com;github.com" : "输入以分号分隔的允许域列表。例如：owncloud.com;github.com",
    ...
    "To create a new account on %s, just click the following link:" : "要在%s创建一个新的账号，只需点击以下链接：",
