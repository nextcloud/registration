OC.L10N.register(
    ...
    "Please provide a valid login name." : "請提供有效的登入名稱。",
    "The login name you have chosen already exists." : "您選擇的登入名稱已存在。",
    "The phone number needs to contain the country code." : "電話號碼必須包含國家代碼。",
    ...
    "This app allows users to self-register a new account using their email address." : "此應用程式允許使用者使用他們的電子郵件地址自行註冊新帳號。",
    ...
    "Enabling \"admin approval\" will prevent registrations from mobile and desktop clients to complete as the credentials cannot be verified by the client until the user was enabled." : "啟用「管理員批准」將會讓行動裝置與桌面客戶端無法完成註冊，因為在啟用使用者前，客戶端無法驗證憑證。",
    "Registered users default group" : "註冊使用者的預設群組",
    "Select group" : "選取群組",
    "Email settings" : "電子郵件設定",
    "Allowed email domain" : "已允許的電子郵件網域",
    ...
    "Regular expression to validate login names" : "用來驗證登入名稱的正規表示式",
    "If configured, login names will be validated through the regular expression. If the validation fails the user is prompted with a generic error. Make sure your regex is working correctly." : "若設定，登入名稱將會透過正規表示式驗證。若驗證失敗，將會對使用者顯示通用錯誤。請確保您的正規表示式可以正確運作。",
    "Show full name field" : "顯示完整名稱欄位",
    "Enforce full name field" : "強制完整名稱欄位",
    "Show phone field" : "顯示電話欄位",
    "Enforce phone field" : "強制電話欄位",
    ...
    "Registration form instructions" : "註冊表單填寫說明",
    "A short message that is shown to the user in the registration process." : "在註冊過程中對使用者顯示的簡短訊息。",
    "Add additional user instructions (e.g. for choosing their login name). If configured the text is displayed in the account creation step of the registration process." : "新增其他使用者說明（例如選擇他們的登入名稱）。若設定，這些文字將會在註冊過程中的帳號建立步驟顯示。",
    "Verification email instructions" : "驗證電子郵件說明",
    "A short message that is shown to the user in the verification email." : "在驗證電子郵件中對使用者顯示的簡短訊息。",
    "Add additional user instructions (e.g. for choosing their login name). If configured the text is embedded in the verification-email." : "新增其他使用者說明（例如選擇他們的登入名稱）。若已設定，文字將會嵌入到驗證電子郵件中。",
    "Blocked email domains" : "已封鎖的電子郵件網域",
    ...
    "Email" : "電子郵件",
    "Continue" : "繼續",
    "Request verification code" : "請求驗證碼",
